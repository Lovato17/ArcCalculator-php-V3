...
class ReportGenerator
{
    ...
    public function generatePDF(array $analysisResults, ?string $clientName = null, array $customNotes = []): array
    {
        try {
            ...
            // Desabilitar bordas de tabela padrão
            ...
            $html = $this->generatePDFContent($analysisResults, $clientName, $customNotes);
            $mpdf->WriteHTML($html);
     ...
     */
    private function generatePDFContent(array $analysisResults, ?string $clientName, array $customNotes = []): string
    {
        ...
        <style>
            ...
            /* Remove ALL table borders - mPDF specific */
            table { border: 0 !important; border-collapse: collapse; border-spacing: 0; }
            table td, table th { border: 0 !important; border-left: 0 !important; border-right: 0 !important; border-top: 0 !important; border-bottom: 0 !important; }
            tr { border: 0 !important; }
            thead, tbody, tfoot { border: 0 !important; }
            
            /* Colors */
            .bg-blue { background-color: #08BED5; }
            .bg-teal { background-color: #005758; }
            .text-blue { color: #08BED5; }
            .text-teal { color: #005758; }
            
            /* Cover Page - TD SYNNEX Official Style */
            ...
            .header-container {
                border-bottom: 2px solid #08BED5;
                padding-bottom: 20px;
                margin-bottom: 40px;
            }
            .header-title {
            ...
            .rec-title { font-weight: bold; font-size: 11px; margin-bottom: 5px; display: block; color: #005758; text-transform: uppercase; }
        ...
        // Header Section (Internal Pages)
        $html .= '
        <div class="header-container">
            <table width="100%" border="0" cellpadding="0" cellspacing="0" style="border:0;border-collapse:collapse;">
                <tr style="border:0;">
                    <td width="70%">
                        <div class="header-title">Relatório de Migração Azure</div>
                    </td>
                    <td width="30%" align="right" style="color: #08BED5; font-size: 10px;">
                        ' . date('d/m/Y') . '
                    </td>
                </tr>
            </table>
        ...
        <div style="background: #f8fcfd; padding: 20px; margin-bottom: 25px; border-left: 4px solid #005758;">
            <div style="font-size: 11px; color: #444; line-height: 1.6; margin-bottom: 15px;">
                Esta análise técnica avaliou <strong>' . $summary['total'] . ' recursos Azure</strong> quanto à viabilidade de migração 
                entre assinaturas e/ou resource groups. A taxa de sucesso esperada para migração direta é de 
            ...
            <table border="0" cellpadding="0" cellspacing="0" style="border:0; width:100%;">
                <tr style="border:0;">
                    ...
                    <td style="border:0; vertical-align: middle;">
                        <span style="background: ' . $viabilidadeColor . '; color: #fff; padding: 4px 12px; font-size: 10px; font-weight: bold; letter-spacing: 1px;">' . $viabilidade . '</span>
                    </td>
                </tr>
            </table>
        </div>';
        ...
        // Non-Movable Resources Section (Enumerated)
        $notMovableResources = array_filter($results, function($r) {
            return $r['status'] === 'not-movable';
        });

        if (!empty($notMovableResources)) {
            $html .= '<div class="section-title" style="color: #D9272E; border-bottom-color: #D9272E;">⚠️ Recursos Não Migráveis (Bloqueantes)</div>';
            ...
            $html .= '<thead style="border:0;">
                <tr style="border:0;">
                    <th width="5%" style="border:0;">#</th>
                    <th width="30%" style="border:0;">Nome do Recurso</th>
                    <th width="30%" style="border:0;">Tipo</th>
                    <th width="35%" style="border:0;">Motivo / Observação</th>
                </tr>
            </thead>
            <tbody style="border:0;">';
            
            $count = 1;
            foreach ($notMovableResources as $nm) {
                $html .= '<tr>
                    <td style="font-weight: bold; color: #D9272E;">' . $count++ . '</td>
        ...
        // Check if there are any custom notes
        $hasCustomNotes = !empty($customNotes);
        
        $html .= '<table class="data-table" border="0" cellpadding="0" cellspacing="0" style="border:0;border-collapse:collapse;">
            <thead style="border:0;">
                <tr style="border:0;">
                    <th width="18%" style="border:0;">Nome do Recurso</th>
                    <th width="22%" style="border:0;">Tipo</th>
                    <th width="12%" style="border:0;">Grupo de Recursos</th>
                    <th width="12%" style="border:0;">Status</th>
                    <th width="18%" style="border:0;">Observações</th>
                    <th width="18%" style="border:0;">Notas Personalizadas</th>
                </tr>
            </thead>
            <tbody style="border:0;">';
        ...
        foreach ($results as $result) {
            ...
            // Define icons using HTML entities supported by standard fonts
            $icon = match($result['status']) {
                'movable' => '&#10004;', // Heavy Check Mark
                'movable-with-restrictions' => '&#9888;', // Warning Sign
                'not-movable' => '&#10008;', // Heavy Ballot X
                default => '?'
            };

            // Define text label without emoji
            ...
            // Get custom note for this resource
            ...
            $html .= '<tr>
                ...
                <td style="font-family: monospace; color: #005758;">' . htmlspecialchars($result['resourceType']) . '</td>
                ...
                <td style="' . ($customNote ? 'background: #f0f9ff; font-style: italic;' : '') . '">' . ($customNote ? htmlspecialchars($customNote) : '-') . '</td>
            </tr>';
        }
        ...
        // Legend
        $html .= '
        <div style="margin-top: 30px; padding: 15px; background: #f8fcfd; border: 1px solid #e0e0e0; border-radius: 0; font-size: 9px;">
        ...
        $html .= '<div class="footer">
            Gerado por Azure Migration Analyzer • TD SYNNEX • ' . date('Y') . '
        </div>';
    ...
    private function createSummarySheet(Spreadsheet $spreadsheet, array $analysisResults, ?string $clientName): void
    {
        ...
        $sheet->getStyle('A3:G3')->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB($blueColor);
        ...
        $sheet->getStyle('A' . $row . ':G' . $row)->getBorders()->getBottom()->setBorderStyle(Border::BORDER_THIN)->getColor()->setRGB($blueColor);
        $row += 2;
    ...
    private function createDetailsSheet(Spreadsheet $spreadsheet, array $analysisResults, array $customNotes = []): void
    {
        ...
        foreach ($results as $result) {
            ...
            $sheet->setCellValue('F' . $row, $result['resourceGroupMove'] ? '✓' : '✗');
            $sheet->setCellValue('G' . $row, $result['subscriptionMove'] ? '✓' : '✗');
            $sheet->setCellValue('H' . $row, $result['regionMove'] ? '✓' : '✗');
            $sheet->setCellValue('I' . $row, $result['notes']);
            
            // Nota personalizada
            $resourceId = base64_encode($result['resourceName'] . '|' . $result['resourceType'] . '|' . $result['resourceGroup']);
            $customNote = $customNotes[$resourceId] ?? '';
            $sheet->setCellValue('J' . $row, $customNote);

            // Estilo baseado no status
            $bgColor = match($result['status']) {
                'movable' => $greenColor,
                'movable-with-restrictions' => $yellowColor,
                'not-movable' => $redColor,
                default => $grayColor
            };
            $textColor = match($result['status']) {
                'movable' => $greenText,
                'movable-with-restrictions' => $yellowText,
                'not-movable' => $redText,
                default => '666666'
            };

            // Aplica estilo à linha inteira (exceto nota personalizada)
            $sheet->getStyle('A' . $row . ':I' . $row)->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB($bgColor);
            $sheet->getStyle('E' . $row)->getFont()->setBold(true);
            $sheet->getStyle('E' . $row)->getFont()->getColor()->setRGB($textColor);
            
            // Estilo da nota personalizada (destaque se houver)
            if ($customNote) {
                $sheet->getStyle('J' . $row)->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB($noteBlueColor);
                $sheet->getStyle('J' . $row)->getFont()->setItalic(true);
            } else {
                $sheet->getStyle('J' . $row)->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB($bgColor);
            }
            
            // Centraliza colunas de check
            $sheet->getStyle('F' . $row . ':H' . $row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
            $sheet->getStyle('F' . $row)->getFont()->getColor()->setRGB($result['resourceGroupMove'] ? $greenText : $redText);
            $sheet->getStyle('G' . $row)->getFont()->getColor()->setRGB($result['subscriptionMove'] ? $greenText : $redText);
            $sheet->getStyle('H' . $row)->getFont()->getColor()->setRGB($result['regionMove'] ? $greenText : $redText);

            // Bordas sutis
            $sheet->getStyle('A' . $row . ':J' . $row)->getBorders()->getBottom()->setBorderStyle(Border::BORDER_THIN)->getColor()->setRGB('E0E0E0');

            // Fonte
            $sheet->getStyle('A' . $row . ':J' . $row)->getFont()->setSize(9)->setName('Segoe UI');
